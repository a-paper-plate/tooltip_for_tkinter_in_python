...
class ToolTip:
    instances:ToolTip=[]
    def __init__(self,root,widget:ToolTip,tooltip_window:tk.Frame|None=None,generate_mouse_controls:bool=True, force_MB1_deletion:bool=False)-> tk.Frame|None:
        """will generate tkinter.Frame for the tooltip if not given one with the master being whatever you give for root (notable for tooltip positioning)"""
        ...
        if generate_mouse_controls:
            if not force_MB1_deletion:
                root.bind('<Button-1>', self._hide_tooltip,add="+")
